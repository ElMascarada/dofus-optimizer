...
function classifyProfileCondition(entry, context) {
  ...
  return {
    ...
    hasCondition: Boolean(entry?.item?.conditions)
  };
}

function conditionSignature(state) {
  if (Number(state.conditionCount || 0) === 0) return 'conditionless';
  if (state.conditionClass === CONDITION_FEASIBILITY.COMPATIBLE) return 'compatible-conditional';
...
function extendConditionState(state, candidate) {
  ...
  return {
    ...
    conditionCount: Number(state.conditionCount || 0) + Number(candidate.hasCondition)
  };
}
...
function keepChoiceDiversity(states, limit, context) {
  ...
  // Protect a conditionless lane inside the existing beam. This is not a
  // second/fallback search: these states are generated by the same expansion
  // and consume the same fixed limit as every other group choice. When the
  // query has only AP/MP hard minimums, keep offensive specialist diversity
  // inside that lane; stronger extra hard minimums keep the legacy ordering so
  // structural constraints retain priority.
  const conditionlessReserve = Math.min(
    limit,
    Math.max(0, Number(context.profile.search.groupOffenseReserve || 0))
  );
  const conditionlessTarget = output.length + conditionlessReserve;
  const conditionlessStates = [...states]
    .filter((state) => Number(state.conditionCount || 0) === 0);
  const hasConditionalCompetition = states.some((state) => Number(state.conditionCount || 0) > 0);
  const hasExtraHardMinimums = [...constraintKeys].some((key) => !['ap', 'mp'].includes(key));
  const conditionlessSpecialistReserve = hasConditionalCompetition && !hasExtraHardMinimums
    ? Math.min(1, Math.max(0, Number(context.profile.search.groupSpecialistReservePerStat || 0)))
    : 0;
  const conditionlessSpecialistKeys = (context.policy.paretoKeys || [])
    .filter((key) => GENERIC_OFFENSE_KEYS.includes(key));
  for (const key of conditionlessSpecialistKeys) {
    if (output.length >= conditionlessTarget || conditionlessSpecialistReserve <= 0) break;
    const bySpecialist = [...conditionlessStates]
      .filter((state) => retentionStat(state, key, constraintKeys) > 0)
      .sort((a, b) => b.objectiveScore - a.objectiveScore
        || b.score - a.score
        || retentionStat(b, key, constraintKeys) - retentionStat(a, key, constraintKeys));
    let kept = 0;
    for (const state of bySpecialist) {
      if (tryKeep(state, { enforceBucket: false })) kept++;
      if (kept >= conditionlessSpecialistReserve || output.length >= conditionlessTarget) break;
    }
  }
  const byConditionlessScore = [...conditionlessStates]
    .sort((a, b) => b.score - a.score || b.objectiveScore - a.objectiveScore);
  for (const state of byConditionlessScore) {
    if (output.length >= conditionlessTarget) break;
    tryKeep(state, { enforceBucket: false });
  }

  // Already-compatible conditional choices remain useful, but they no longer
  // share the conditionless lane and therefore cannot evict it wholesale.
  ...
  const compatibleStates = [...states]
    .filter((state) => Number(state.conditionCount || 0) > 0
      && state.conditionClass === CONDITION_FEASIBILITY.COMPATIBLE)
    .sort((a, b) => b.score - a.score || b.objectiveScore - a.objectiveScore);
