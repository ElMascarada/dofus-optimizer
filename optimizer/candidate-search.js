...
function keepChoiceDiversity(states, limit, context) {
  ...
  // fixed lane, retain strong representatives for each Pareto dimension before
  // filling by aggregate score so a low-isolation specialist cannot disappear
  // solely because conditional alternatives rank higher in the shared beam.
  ...
  const conditionlessSpecialistReserve = Math.max(
    0,
    Number(context.profile.search.groupSpecialistReservePerStat || 0)
  );
