...
function keepChoiceDiversity(states, limit, context) {
  ...
  // and consume the same fixed limit as every other group choice. Within that
  // fixed lane, retain strong representatives for each Pareto dimension only
  // when conditional alternatives are competing for the same beam capacity.
  ...
  const hasConditionalCompetition = conditionlessStates.length < states.length;
  const conditionlessSpecialistReserve = hasConditionalCompetition
    ? Math.max(0, Number(context.profile.search.groupSpecialistReservePerStat || 0))
    : 0;
  for (const key of context.policy.paretoKeys || []) {
    if (output.length >= conditionlessTarget || conditionlessSpecialistReserve <= 0) break;
    const structuralConstraint = constraintKeys.has(key);
    const bySpecialist = [...conditionlessStates]
      .filter((state) => retentionStat(state, key, constraintKeys) > 0)
      .sort((a, b) => {
        const statDelta = retentionStat(b, key, constraintKeys) - retentionStat(a, key, constraintKeys);
        if (structuralConstraint) return statDelta || b.score - a.score || b.objectiveScore - a.objectiveScore;
        return b.objectiveScore - a.objectiveScore || b.score - a.score || statDelta;
      });
    let kept = 0;
