...
if (!witnessEval.result) throw new Error(`OWNER_FIRE_WATER_SMALL_AUTO witness rejected: ${witnessEval.reason}`);
...
if (!best) throw new Error('OWNER_FIRE_WATER_SMALL_AUTO returned no legal build');

const comparison = compareCompleteEquipmentBuildResults(witnessEval.result, best);
console.log(`OWNER_FIRE_WATER_SMALL_AUTO_MS=${elapsedMs.toFixed(1)}`);
console.log(`OWNER_FIRE_WATER_SMALL_AUTO_SEARCH_MIN=${best.syntheticOffense?.minimumScore ?? 'NA'}`);
console.log(`OWNER_FIRE_WATER_SMALL_AUTO_WITNESS_MIN=${witnessEval.result.syntheticOffense?.minimumScore ?? 'NA'}`);
console.log(`OWNER_FIRE_WATER_SMALL_AUTO_VERDICT=${comparison > 0 ? 'WITNESS_BETTER' : comparison < 0 ? 'SEARCH_BETTER' : 'EQUAL'}`);
console.log(`OWNER_FIRE_WATER_SMALL_AUTO_DIRECT=${JSON.stringify(output.diagnostics || null)}`);
console.log(`OWNER_FIRE_WATER_SMALL_AUTO_ITEMS=${(best.items || []).map((item) => item?.name || item?.id).join(' | ')}`);
if (comparison > 0) throw new Error('OWNER_FIRE_WATER_SMALL_AUTO quality regression: owner witness still beats optimizer top');
