...
const CASES = Object.freeze({
  tri: {
    label: 'TRI_EARTH_FIRE_AIR_LARGE_NO_CRIT_12_5',
    constraints: { ap: 12, mp: 5 },
    syntheticOffense: { elements: ['earth', 'fire', 'air'], profiles: ['large'], critMode: 'no_crit' }
  },
  multi: {
    label: 'MULTI_LARGE_NO_CRIT_12_5',
    constraints: { ap: 12, mp: 5 },
    syntheticOffense: { elements: ['multi'], profiles: ['large'], critMode: 'no_crit' }
  }
});

const selectedCase = String(process.argv[2] || '').trim().toLowerCase();
const probe = CASES[selectedCase];
if (!probe) {
  console.error(`USAGE=node ${new URL(import.meta.url).pathname} <tri|multi>`);
  process.exitCode = 64;
} else {
  ...
  console.log(`REQUEST=${JSON.stringify({
    ...
    syntheticOffense: probe.syntheticOffense,
    topN: common.topN,
    searchProfile: common.searchProfile
  })}`);

  const prefilterStartedAt = performance.now();
  const prefilter = buildEquipmentCandidatePools(common);
  const prefilterMs = performance.now() - prefilterStartedAt;
  console.log(`PREFILTER_MS=${prefilterMs.toFixed(1)}`);
  ...
  const output = searchEquipmentRequest(common);
