...
function candidateConstraintOrder(profile, constraints = {}) {
  ...
  return {
    structural,
    other,
    offense: Number(profile.rankScore || 0)
  };
}
...
/**
 * Exhaustive zero-result correction path.
 *
 * Completeness rule: every optimizer-eligible item remains reachable. Candidate
 * policy scores are used only to order DFS exploration; no rank, beam, top-N or
 * diversity cutoff removes a branch. Branches are discarded only for certified
 * structural impossibility, a constraint upper envelope that cannot reach a
 * minimum, or an admissible offensive upper bound below an already-valid
 * incumbent.
 */
export function searchConstraintCompletenessRescue({
  ...
  for (const rule of SLOT_RULES) {
    ...
    groups.push({
      id: rule.id,
      missing,
      profiles: sortProfilesForExploration(available, constraints)
    });
  }
  ...
  function progress(label = 'rescue contraintes') {
    ...
    onProgress({
      phase: 'constraint-rescue',
      label,
      nodes,
      visited: valid,
      pruned,
      best: incumbent?.score || 0,
      partialResults: incumbent ? [incumbent] : null
    });
  }
  ...
  function remainingGroups(groupIndex, picksLeftInCurrent = 0) {
    ...
    if (picksLeftInCurrent > 0) {
      const current = orderedGroups[groupIndex];
      remaining.push({ id: current.id, missing: picksLeftInCurrent });
    }
  ...
  function evaluateLeaf() {
    // A fresh scenario object gives the evaluator's WeakMap cache a collectible
    // key per leaf. The rescue therefore does not retain every evaluated build
    // in the long-lived normal-search scenario cache.
