...
function linearScoreTableForElement({ stats, availableAp, element, profiles, critMode, minQ, maxQ }) {
  ...
  const lines = atZero.requestedProbes.map((probe, index) => ({
    intercept: Number(probe.totalApBudgetScore || 0),
    slope: Number(atOne.requestedProbes[index]?.totalApBudgetScore || 0) - Number(probe.totalApBudgetScore || 0)
  }));
...
function bestSecondQByBudget({ minimumQ, maxQ, maximumBudget, softCaps, table }) {
...
export function optimizeSyntheticCharacteristicsTwoElementLinear(options = {}) {
  const {
    baseStats = {},
    points = 995,
    scrolled = {},
