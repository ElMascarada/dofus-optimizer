...
const ELEMENT_LABELS = { earth: 'Terre', fire: 'Feu', water: 'Eau', air: 'Air', multi: 'Multi' };
...
function displayTurnsForBuild(build) {
  const turns = build?.combatPlan?.objective?.activeTurns;
  return Array.isArray(turns) && turns.length ? turns : activeTurnsForMode();
}
...
function syncTurnInputs() {
  ...
  for (const row of document.querySelectorAll('.spell-row')) {
    ...
    for (const turn of [1, 2, 3]) {
      ...
      if (input) input.disabled = combat || !allowed.has(turn);
    }
  }
}

function automaticSpellPool() {
  if (!spellData || !breedSelect.value) return [];
  return combatSpellsForElement(spellData, breedSelect.value, $('#combat-element')?.value || 'multi');
}

function updateCombatSummary() {
  const target = $('#combat-spell-summary');
  if (!target || !spellData || !breedSelect.value) return;
  const pool = automaticSpellPool();
  const offensive = pool.filter((spell) => (spell.hits || []).length > 0).length;
  const supports = pool.filter((spell) => !(spell.hits || []).length && (spell.combatModifiers || []).length).length;
  const variants = pool.filter((spell) => spell.isVariant).length;
  const element = ELEMENT_LABELS[$('#combat-element')?.value || 'multi'] || 'Multi';
  target.textContent = `${element} · ${offensive} sorts offensifs · ${supports} supports purs · ${variants} variantes dans le pool automatique.`;
}
...
function syncObjectiveControls() {
  ...
  optimizeButton.textContent = worker ? 'Arrêter le calcul' : combat ? 'Optimiser automatiquement' : 'Optimiser le stuff';
  syncTurnInputs();
  updateCombatSummary();
}
...
function renderSpellRows() {
  ...
  spellList.innerHTML = visibleSpells.map((spell) => {
    ...
    const modifier = spell.combatModifiers?.length ? '<span class="spell-support-badge">Buff/debuff</span>' : '';
    return `
      <div class="spell-row" data-spell-id="${spell.id}">
        <label class="check"><input type="checkbox" class="spell-enabled"> <span>${escapeHtml(spell.name)}</span>${variant}${modifier}</label>
...
function readCombatObjective() {
  return {
    element: $('#combat-element')?.value || 'multi',
    turnMode: $('#turn-mode')?.value || 't1',
...
function readScenario() {
  ...
  for (const [key, value] of Object.entries(fields)) if (value !== undefined) scenario[key] = Math.max(0, value);
  return scenario;
}
...
function renderResult(build, rank, index) {
  ...
  const scoreLabel = build.combatPlan ? 'meilleure rotation' : 'score objectif';
...
function statSection(build, title, entries) {
  return `<section class="detail-section"><h3>${title}</h3><div class="detail-stat-grid">${entries.map(([key, suffix = '']) => statTile(build, key, suffix)).join('')}</div></section>`;
}
...
function renderDetailedEquipment(build) {
  ...
  return [...build.items].sort((a, b) => (slotRank.get(a.slot) ?? 99) - (slotRank.get(b.slot) ?? 99)).map((item) => {
    const image = item.imageUrl ? `<img src="${escapeHtml(item.imageUrl)}" alt="" loading="lazy">` : '<div class="gear-placeholder"></div>';
    const subtype = item.slotSubtype === 'prysmaradite' ? ' · Prysmaradite' : '';
    return `<div class="detail-gear-row">${image}<div><small>${SLOT_LABELS[item.slot] || item.slot}</small><strong>${escapeHtml(item.name)}${subtype}</strong></div><em>${fmTextFor(build, item)}</em></div>`;
  }).join('');
}
...
function renderActiveSets(build) {
  ...
  return build.activeSets.map((set) => `<div class="set-card"><header><strong>${escapeHtml(set.name || set.setId)}</strong><span>${set.count} items</span></header><div class="set-bonus-list">${renderSetBonusStats(set.bonus)}</div></div>`).join('');
}

function spellIconUrl(spell) {
  const iconId = Number(spell?.iconId || 0);
  return iconId ? `https://api.dofusdu.de/dofus3/v1/img/spell/${iconId}-96.png` : '';
}

function renderSpellTurnChips(spell) {
  return [1, 2, 3].map((turn) => `<span class="spell-turn-chip"><small>T${turn}</small><b>${fmt(spell.perTurn?.[turn]?.expected)}</b></span>`).join('');
}
...
function renderSpellDamageCards(build) {
  ...
  const cards = spells.map((spell) => {
    ...
    const icon = iconUrl ? `<span class="spell-damage-icon"><img src="${escapeHtml(iconUrl)}" alt="" loading="lazy"><span>✦</span></span>` : '<span class="spell-damage-icon"><span>✦</span></span>';
    ...
    return `<details class="spell-damage-card"><summary>${icon}<span class="spell-damage-meta"><strong>${escapeHtml(spell.name)}</strong><small>${fmt(spell.apCost)} PA · moyenne globale T1–T3</small></span><span class="spell-turn-chips">${renderSpellTurnChips(spell)}</span><b class="spell-damage-average">${fmt(spell.averageDamage)}</b></summary><div class="spell-damage-expanded"><div><span>Plage normale moyenne</span><b>${fmt(spell.normal?.[0])} – ${fmt(spell.normal?.[1])}</b></div><div><span>Plage critique moyenne</span><b>${fmt(spell.critical?.[0])} – ${fmt(spell.critical?.[1])}</b></div><div><span>Chance de critique moyenne</span><b>${fmt(spell.critChancePct)}%</b></div><div class="spell-turn-details">${perTurnDetails}</div></div></details>`;
  }).join('');
  return `<section class="detail-section spell-damage-section"><div class="spell-damage-heading"><div><h3>Dégâts moyens par sort</h3><p>Moyenne globale puis moyenne T1/T2/T3 pour visualiser les effets temporels comme le Nébuleux.</p></div><small>Cliquer sur un sort pour détailler</small></div><div class="spell-damage-list">${cards}</div></section>`;
}

function renderCombatPlan(build) {
  const plan = build?.combatPlan;
  if (!plan?.sequence?.length) return '';
  const selectedTurns = Array.isArray(plan.objective?.activeTurns) ? plan.objective.activeTurns : [1];
  const blocks = selectedTurns.map((turn) => {
    const entries = plan.sequence.filter((entry) => Number(entry.turn) === Number(turn));
    ...
    return `<div class="combat-turn-block"><header><strong>T${turn}</strong><span>${fmt(plan.perTurn?.[turn])} dégâts</span></header>${rows}</div>`;
  }).join('');
  const target = plan.objective?.targetMode === 'zone' ? `Zone · ${plan.objective.areaTargets} cibles` : '1 cible';
  return `<section class="detail-section combat-plan-section"><div class="spell-damage-heading"><div><h3>Meilleure séquence trouvée</h3><p>${target} · ${selectedTurns.map((turn) => `T${turn}`).join(' + ')} · dégâts moyens</p></div><b>${fmt(plan.totalDamage)}</b></div><div class="combat-turn-list">${blocks}</div></section>`;
}

function ensureBuildModal() {
  if (modalReady) return;
  document.body.insertAdjacentHTML('beforeend', '<div id="build-modal" class="build-modal" aria-hidden="true"><div class="build-modal-backdrop" data-close-modal></div><div class="build-modal-panel" role="dialog" aria-modal="true" aria-labelledby="build-modal-title"><button class="build-modal-close" type="button" data-close-modal aria-label="Fermer">×</button><div id="build-modal-content"></div></div></div>');
  const modal = $('#build-modal');
  modal.addEventListener('click', (event) => { if (event.target.closest('[data-close-modal]')) closeBuildModal(); });
  document.addEventListener('keydown', (event) => { if (event.key === 'Escape' && modal.classList.contains('is-open')) closeBuildModal(); });
  modalReady = true;
}
...
function openBuildModal(build, rank) {
  ...
  const warnings = build.warnings?.length ? `<div class="detail-warning">Hypothèses / avertissements : ${build.warnings.map(escapeHtml).join(' · ')}</div>` : '';
  $('#build-modal-content').innerHTML = `
    <header class="detail-header"><div><span class="rank">#${rank}</span><h2 id="build-modal-title">Fiche complète du stuff</h2></div><div class="detail-score"><b>${fmt(build.score)}</b><span>${build.combatPlan ? 'meilleure rotation' : 'score objectif'}</span></div></header>
    <div class="detail-turns">${turns}</div>${warnings}
    <div class="build-detail-layout">
      <div class="detail-column">${statSection(build, 'Statistiques principales', [['ap'], ['mp'], ['range'], ['vit'], ['power'], ['crit', '%'], ['wisdom'], ['summons']])}${statSection(build, 'Caractéristiques', [['earth'], ['fire'], ['water'], ['air']])}${statSection(build, 'Secondaires', [['initiative'], ['prospecting'], ['lock'], ['dodge'], ['apReduction'], ['mpReduction'], ['apParry'], ['mpParry'], ['heals']])}</div>
      <div class="detail-column detail-equipment-column"><section class="detail-section"><h3>Équipement & FM</h3><div class="detail-gear-list">${renderDetailedEquipment(build)}</div></section><section class="detail-section"><h3>Caractéristiques automatiques</h3><pre>${JSON.stringify(build.characteristics, null, 2)}</pre></section></div>
      <div class="detail-column">${statSection(build, 'Dommages', [['damage'], ['damageNeutral'], ['damageEarth'], ['damageFire'], ['damageWater'], ['damageAir'], ['critDamage'], ['spellDamagePct', '%'], ['weaponDamagePct', '%'], ['pushbackDamage'], ['trapDamage'], ['trapPower']])}${statSection(build, 'Résistances %', [['resNeutral', '%'], ['resEarth', '%'], ['resFire', '%'], ['resWater', '%'], ['resAir', '%'], ['critResistance'], ['pushbackResistance']])}${statSection(build, 'Résistances fixes', [['fixedResNeutral'], ['fixedResEarth'], ['fixedResFire'], ['fixedResWater'], ['fixedResAir']])}<section class="detail-section"><h3>Panoplies équipées</h3>${renderActiveSets(build)}</section></div>
    </div>${renderCombatPlan(build)}${renderSpellDamageCards(build)}`;
  const modal = $('#build-modal');
...
function renderBuilds(builds, emptyText) {
  ...
  results.innerHTML = displayedBuilds.length ? displayedBuilds.map((build, index) => renderResult(build, index + 1, index)).join('') : `<div class="empty">${emptyText}</div>`;
}

results.addEventListener('click', (event) => {
  const card = event.target.closest('.result-card');
  if (!card || event.target.closest('details, summary, a, input, select')) return;
  openBuildModal(displayedBuilds[Number(card.dataset.buildIndex)], Number(card.dataset.buildIndex) + 1);
});
...
function setIdleState() {
  ...
  optimizeButton.textContent = isCombatMode() ? 'Optimiser automatiquement' : 'Optimiser le stuff';
}
...
function stopSolver() {
  ...
  if (latestPartialResults.length) {
    ...
    diagnostics.textContent = `Recherche arrêtée · ${latestPartialResults.length} meilleurs stuffs conservés · ${Number(latestProgress?.nodes || 0).toLocaleString('fr-FR')} nœuds parcourus`;
  } else {
...
function runSolver() {
  ...
  const combatObjective = readCombatObjective();
  const classSpells = combatMode ? automaticSpellPool() : spellsForBreed(spellData, breedSelect.value);
  if (combatMode && !classSpells.some((spell) => (spell.hits || []).length > 0)) {
    results.innerHTML = `<div class="empty">Aucun sort offensif ${escapeHtml(ELEMENT_LABELS[combatObjective.element] || combatObjective.element)} disponible pour cette classe.</div>`;
    return;
  }
  ...
  results.innerHTML = '<div class="empty">Recherche en cours : panoplies, slots offensifs puis meilleure rotation…</div>';
  diagnostics.textContent = combatMode
    ? `${ELEMENT_LABELS[combatObjective.element]} · ${activeTurnsForMode().map((turn) => `T${turn}`).join(' + ')} · ${classSpells.length} sorts disponibles…`
...
function initDefaults() {
  ...
  $('#turn-mode').value = 'sum';
  $('#turn-mode').addEventListener('change', () => { syncTurnInputs(); updateCombatSummary(); });
  ...
  $('#combat-element').addEventListener('change', updateCombatSummary);
...
async function initData() {
  ...
  try {
    ...
    const variantRefs = Number(spellData.coverage?.variantSpellRefs || 0);
    ...
    const variantStatus = variants > 0 ? `${variants.toLocaleString('fr-FR')} variantes` : `0 variante chargée${variantRefs ? ` (${variantRefs} refs détectées)` : ' · snapshot à régénérer'}`;
    dataStatus.textContent = `${dataset.items.length.toLocaleString('fr-FR')} équipements · ${dataset.sets.length.toLocaleString('fr-FR')} panoplies · ${spellData.spells.length.toLocaleString('fr-FR')} sorts de combat · ${variantStatus} · ${buffSpells.toLocaleString('fr-FR')} sorts avec buff/debuff · ${support.toLocaleString('fr-FR')} supports purs · calcul 100% local`;
    initBreedSelect();
